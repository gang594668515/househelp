...
function updatePhone() {
    var ok = $('#updatePhoneForm').parsley().isValid({force: true});
    if (!ok) {
        return;
    }
    ...
    $.ajax({
        ...
        success: function (data, textStatus) {
            if (data.rspCode == '000000') {
                $("#phoneError").hide();
                $("#updatePhoneBtn").attr("aria-hidden", "true");
                $("#updatePhoneBtn").attr("data-dismiss", "modal");
                $("#updatePhoneForm")[0].reset();
                locationUrl('/userList','userList');
                toastr.success('手机号修改成功！', '操作成功')
            } else {
                $("#phoneError").show();
                $("#phoneError").html(data.rspMsg);
                $("#updatePhoneBtn").removeAttr("aria-hidden");
                $("#updatePhoneBtn").removeAttr("data-dismiss");
            }
        }
    });
}
function updateEmail() {
    var ok = $('#updateEmailForm').parsley().isValid({force: true});
    if (!ok) {
        return;
    }
    ...
    $.ajax({
        ...
        success: function (data, textStatus) {
            if (data.rspCode == '000000') {
                $("#emailError").hide();
                $("#updateEmailBtn").attr("aria-hidden", "true");
                $("#updateEmailBtn").attr("data-dismiss", "modal");
                $("#updateEmailForm")[0].reset();
                locationUrl('/userList','userList');
                toastr.success('邮箱修改成功！', '操作成功')
            } else {
                $("#emailError").show();
                $("#emailError").html(data.rspMsg);
                $("#updateEmailBtn").removeAttr("aria-hidden");
                $("#updateEmailBtn").removeAttr("data-dismiss");
            }
        }
    });
}
